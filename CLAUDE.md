...
- `com.vortex.vortexweb.notifications` — `NotificationService` interface (`send(to, subject, body)`) abstracting outbound client emails, with `EmailNotificationService` as the sole implementation, calling the [Resend](https://resend.com) API (`POST {base-url}/emails`, `Authorization: Bearer {api-key}`, body `{from, to: [to], subject, text}`) via a `RestClient`, configured from `vortex.notifications.email.*`. Callers (e.g. `BookingController`, `AppointmentAdminController`) depend only on the interface; tests that don't care about emails should `@MockitoBean` it rather than let real submissions hit the (fake) configured provider URL.
...
- `EmailNotificationService.send()` catches and logs (not rethrows) any exception from the outbound call. Found by actually running the app: before this fix, a failed/unreachable send took the whole request down with a 500 *after* the underlying write (appointment save, confirm, decline, etc.) had already committed — so a real booking would succeed in the DB but look like an error to the user. Every `NotificationService` caller relies on `send()` never throwing; don't reintroduce a propagating exception here without updating all call sites.
- **Real email provider is Resend.** `vortex.notifications.email.api-key` reads from the `RESEND_API_KEY` environment variable (`${RESEND_API_KEY:changeme}`), which is never committed. For local dev, an environment variable set via the interactive session doesn't propagate into this agent's own tool process, so the working pattern here is a git-ignored `src/main/resources/application-local.properties` (see the committed `application-local.properties.example` for the shape) activated with `-Dspring-boot.run.profiles=local`. **Never** write a real API key into the `.example` file or any other tracked file — only into `application-local.properties`.
- `vortex.notifications.email.from-address` currently uses Resend's shared sandbox sender (`onboarding@resend.dev`), which only delivers to the email address on the Resend account that owns the API key — swap for a verified-domain address before sending to real clients.
...
- CI (`.github/workflows/ci.yml`) runs `./mvnw test` on `ubuntu-latest` for every PR and push to `main`. GitHub-hosted runners come with Docker already running, so `spring-boot-docker-compose` starting Postgres for tests works there the same as locally — no extra CI-specific database setup needed. Branch protection on `main` requires this `test` check to pass (and the branch to be up to date) and requires a PR before merging — see repo Settings → Branches for the exact rule.
