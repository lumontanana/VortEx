...
- `com.vortex.vortexweb.admin.AppointmentAdminController` — lists `PENDING` appointments and lets the artist confirm (→ `CONFIRMED`, rejected with 409 if it now overlaps another `CONFIRMED` appointment) or decline (→ `DECLINED`) each one, emailing the client either way. Lives in `admin` (not `booking`) per the "admin controllers live in `admin`" convention, even though `Appointment` itself is a `booking` type.
